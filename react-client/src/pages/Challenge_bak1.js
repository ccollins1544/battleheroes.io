...
const Challenge = () => {
  ...
  const updateGame = (game_status=userState.game_status) => {
    ...
    // =========================[---Accept?---]=========================================
    if(game_status === 1){ 
      API.getUserById(userState.user_id)
      .then(userResponse => {
        let { games, active_game } = userResponse.data;
        ...
        if(possibleGameInvites){

          API.getGameById(possibleGameInvites)
            .then(game => {
    ...
    } else if(pendingGameID.length === 0 && game_status < 2){
